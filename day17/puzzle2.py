...
def get_neighbors(u):
    for DIR in [UP, DOWN, RIGHT, LEFT]:
        ...
        if DIR != BACK_STEP[u[2]]:
            ...
            cords_x = u[0] + shift[0]
            cords_y = u[1] + shift[1]
            if 0 <= cords_x < len(input) and 0 <= cords_y < len(input[0]) and 4 <= u[3] <= 10:
                if cords_x != len(input)-1 or cords_y != len(input[0])-1:
                    yield (cords_x, cords_y, DIR, new_blocks_count)
                else:
                    if new_blocks_count >= 4:
                        yield (cords_x, cords_y, DIR, new_blocks_count)
            elif 0 <= cords_x < len(input) and 0 <= cords_y < len(input[0]) and u[3] < 4 and DIR == u[2]:
                if cords_x != len(input)-1 or cords_y != len(input[0])-1:
                    yield (cords_x, cords_y, DIR, new_blocks_count)
                else:
                    if new_blocks_count == 4:
